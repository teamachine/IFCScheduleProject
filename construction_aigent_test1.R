...
#prctbl <- as.data.frame(read_xlsx("DemoPrecMat25-1.xlsx", sheet = "Sheet1"))
prctbl <- as.data.frame(read_xlsx("precmat_construction_lil_building_4x3.xlsx")) #reading original matrix
prctbl

#
prctbl1 <- data.frame(
  element <- prctbl["Object's IfcClass"],
  elemguid<-prctbl["Object's Guid"],
  #elemindex <- prctbl["elemindex"],  #indexes here are not necessary bc not case sensitive search
  volelement <- prctbl["Object's Total Volume"],
  precedent <- prctbl["Precedence's IfcClass"],
  precguid<-prctbl["Precedence's Guid"],
  #precindex <- prctbl["precindex"],
  volpreced <- prctbl["Precedence's Total Volume"],
...
colnames(prctbl1)<-c("element", "objguid", "volelement", "precedent", "precguid", "volpreced")
e3e<-prctbl1
...
length(uniquelementdf[,1])

uniquelementdf[,"uniqueindex"] <- NA  #create new empty column
uniquelementdf <- uniquelementdf[, c(4,1,2,3)]   #shift position of column to the beginning of the table
for (i in 1:length(uniquelementdf[,1])){
  uniquelementdf[i,1] <- i
}                                                #add index

e3e[,c("elemindex","precindex")] <- NA   #create two new empty columns in ee
e3e <- e3e[, c(7,1,2,3,8,4,5,6)]          #move the columns to positions 1 and 5

#e3e[[1]] <- uniquelementdf[[1]][match(e3e[[3]], uniquelementdf[[2]])]   #finds, for each GUID in e3e column 3, its position in uniquelementdf column 3 — then uses that to pull the corresponding index from uniquelementdf column 1.
e3e[["elemindex"]] <- uniquelementdf[["uniqueindex"]][match(e3e[["objguid"]], uniquelementdf[["uniqueguid"]])]    #as above, but with names instead of col numbers
e3e[["precindex"]] <- uniquelementdf[["uniqueindex"]][match(e3e[["precguid"]], uniquelementdf[["uniqueguid"]])]


# sheets2 <- list("MainTable" = ee, "RefTable" = uniquelementdf)
# write_xlsx(sheets2, "C:\\Users\\melod\\Documents\\School\\BIM A+ 2023\\BIM A+ 7\\BIM A+ 7 Thesis\\IFCProblem\\IFCScheduleProject\\Tables_FJ.xlsx")
...
prctbl2 <- subset(e3e, select = -c(objguid, precguid))
ee<-prctbl2
...
t<- as.data.frame(read_xlsx("teams.xlsx", sheet = "constrteams3"))
row.names(t)<-c("A", "B", "C")
t
...
# plot(generations,normalmeancosts, type="l", col="black", lwd=3, ylim=c(-0.01,0.8), xlab="Generations", ylab="Fitness Value (normalized costs)")
# lines(generations,normalmincosts, col="orange", lwd=3)
# title("Mean and best fitness - costs")
# legend(50,0.8,c("mean","best"), lwd=c(3,3), col=c("black","orange"), y.intersp=1.5)
# 
# plot(generations,normalmeantimes, type="l", col="black", lwd=3, ylim=c(-0.01,0.8), xlab="Generations", ylab="Fitness Value (normalized time)")
# lines(generations,normalmintimes, col="orange", lwd=3)
# title("Mean and best fitness - time")
# legend(50,0.8,c("mean","best"), lwd=c(3,3), col=c("black","orange"), y.intersp=1.5)
...
plot(schedprobb3[[1]]$value, col= gray(0.8) , pch = 19, xlab="Time", ylab="Cost", xlim=c(626,652), ylim=c(55500,55690), frame.plot = TRUE )
...
legend(644, 55690, legend=c('Gen1', 'Gen10', 'Gen50', 'Gen75', 'Gen100', 'FinalPareto'), pch=c(19, 19, 19, 19, 19,1), 
